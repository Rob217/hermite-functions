...
test_data_analytic = [
    ...
    (1, np.array([-1, -0.5, 0]), np.array([-0.64428837, -0.46871702, 0.0])),
]
...
def test_analytic_all_n():
    ...
    np.testing.assert_array_equal(
        hermite_functions(n, x, all_n=True, method="analytic"), psi_expected
    )
...
def test_direct_all_n():
    ...
    np.testing.assert_array_equal(
        hermite_functions(n, x, all_n=True, method="direct"), psi_expected
    )
...
def test_simple(n, x):
    assert hermite_functions(n, x, all_n=False) == pytest.approx(
        hermite_functions(n, x, all_n=False, method="direct")
    )


@pytest.mark.parametrize("n,x", test_data)
def test_all_n(n, x):
    assert hermite_functions(n, x, all_n=True) == pytest.approx(
